  ...
  return (
    <table className="w-full m-auto border-b border-[#e0e0e0] relative text-sm">
      ...
      {cart.line_items.map((product) => (
        <tr key={product.id}>
          <td className="product py-4 w-3/12">
            <div className="max-[360px]:min-w-[100px] max-[360px]:w-[100px] text-center">
              <img src={product.image.url} alt={product.name} />
            </div>
          </td>
          <td className="product px-4 name text-start">
            <div>
              ...
              <div className="actions hidden max-[560px]:inline-block max-[560px]:mt-3">
                <div
                  className={`${
                ...
                >
                  <button
                    ...
                    onClick={() =>
                      updateQuantity(product.id, product.quantity, "remove")
                    }
                  ...
                  <button
                    className="disabled:bg-[#cccccc] font-semibold p-0 w-min-auto"
                    onClick={() =>
                      updateQuantity(product.id, product.quantity, "add")
                    }
                    disabled={status === "loading"}
                  >
                    &#43;
          ...
          <td className="quantity text-end max-[560px]:hidden">
            <div
              className={`${
                status === "loading" && "bg-[#cccccc]"
              } text-white bg-black hover:shadow-md flex flex-row justify-between w-[92px] ml-auto items-center py-[2px] px-[10px]`}
          ...
          <td className="subtotal w-[25%] text-end relative">
            <span
